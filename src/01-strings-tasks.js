...
function extractNameFromTemplate(str) {
  return str.split(', ').slice(1).join('').slice(0, -1);
}
...
function encodeToRot13(str) {
  ...
  const altLetters = 'NOPQRSTUVWXYZABCDEFGHIJKLMnopqrstuvwxyzabcdefghijklm';

  for (let i = 0; i < str.length; i += 1) {
    if (letters.includes(str[i])) {
      result = `${result}${altLetters[letters.indexOf(str[i])]}`;
    } else {
      result = `${result}${str[i]}`;
    }
  }
